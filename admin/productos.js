...
$.ajax({
    ...
    success: function(e) {
        //  console.log(e)
        try { productos = JSON.parse(e) } catch (e) { productos = e }
...
function armarProducto(prod) {
    if (prod == null) {
        ...
        html += '         <div class="row">'
        html += '              <div class="col-3">'
        html += '                  <img src="' + rolesEmpleados[prod.id_rol_empleado] + '">'
        html += '             </div>'
        html += '             <div class="col-7">' + prod.nombre_producto + ' - <span>$' + prod.precio + '</span></div>'
        html += '             <div class="col-2">'
        html += '                   <i class="fa fa-pencil-alt" onclick="mostrarAgregarVerProducto(' + prod.id + ')"></i>'
        html += '                </div>'
        html += '          </div>'
        html += '       </li>'
        html += '       <hr>';
